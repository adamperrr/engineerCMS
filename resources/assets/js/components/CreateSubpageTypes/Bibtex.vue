<template>
    <div>
        ...
        <div class="form-group">
            <label for="bibtexFile">Wybierz plik BibTeX spośród załadowanych plików:</label>
            <div class="input-group">
                <select class="form-control" id="bibtexFile" name="bibtexFile" v-model="selectedFileId" required>
                    <option value="0">---</option>
                    <option v-for="(option, index) in filesOptions"
                            v-bind:value="option.id">
                        {{ option.name }}
                    </option>
                </select>
                <span class="input-group-btn">
                    <button type="button" class="btn btn-primary" id="bibtexFilesRefresh" v-on:click="fetchFilesOptions">
                        <span class="glyphicon glyphicon-refresh"></span>
                    </button>
                </span>
            </div>
            <p>Jeżeli plik BibTeX nie został dodany możesz dodać go <a target="_blank" :href="appurl+'/admin/upload'">pod tym linkiem</a>, a następnie odświeżyć listę.</p>
